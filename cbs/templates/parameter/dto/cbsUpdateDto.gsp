...
/**
 * ${tableDefine.cnname} 默认UpdateDTO
 *
 * @author ${config.author}
 * @date ${config.nowDate}
 */
@Data
@Schema(description = "${tableDefine.cnname} 默认UpdateDTO")
public class ${tableDefine.id}UpdateDTO implements Serializable {
    private static final long serialVersionUID = 42L;

<%
    columns.each{
        def colDataType=tableNameUtil.getDataType(it.columnType, it.length, it.decimalDigits);
        if(!it.getCanBeNull() && !"createdBy,createdAt,".contains(it.dataName)) {
